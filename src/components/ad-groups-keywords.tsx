...
const keywordsData = [
  { keyword: 'Keyword 1', clicks: 100, impressions: 1000, ctr: 10, avgCpc: 0.5, cost: 50 },
  { keyword: 'Keyword 2', clicks: 150, impressions: 1500, ctr: 10, avgCpc: 0.55, cost: 82.5 },
  { keyword: 'Keyword 3', clicks: 200, impressions: 2000, ctr: 10, avgCpc: 0.52, cost: 104 },
  { keyword: 'Keyword 4', clicks: 180, impressions: 1800, ctr: 10, avgCpc: 0.54, cost: 97.2 },
]
...
function renderTable (data: any[], totals: any, itemKey: string)
{
  return (
      ...
      <CardContent className="p-0">
        <Table>
          <TableHeader className="w-full">
            <TableRow>
              <TableHead className='w-1/6'>{ itemKey === 'group' ? 'Ad Group' : 'Keyword' }</TableHead>
        ...
        <div className="relative h-[600px] overflow-auto">
          <Table className="w-full">
            ...
            <TableBody>
              { data.map((row, index) => (
                <TableRow key={ index }>
                  <TableCell className='w-1/6'>{ row[ itemKey ] }</TableCell>
...
export default function AdGroupsKeywords ({ campaign }: { campaign: string })
{
  const adGroupsTotals = calculateTotals(adGroupsData)
  const keywordsTotals = calculateTotals(keywordsData)
  ...
  return (
    <Tabs defaultValue="adGroups">
      ...
      <TabsContent value="adGroups">
        { renderTable(adGroupsData, adGroupsTotals, 'group') }
      </TabsContent>
      <TabsContent value="keywords">
        { renderTable(keywordsData, keywordsTotals, 'keyword') }
      </TabsContent>
    </Tabs>
  )
}
